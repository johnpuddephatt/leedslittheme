  @if ($secondaryNavigation)
    <div class="bg-black pb-2 pt-3 text-white md:border-b md:bg-white md:text-black lg:pb-3 lg:pt-6">
        ...
        <nav class="hidden lg:block">
          <ul class="flex flex-row items-center gap-8">
            @foreach ($secondaryNavigation as $item)
              <li>
                <a class="{{ $item->classes ?? '' }} inline-block" href="{{ $item->url }}">{!! $item->label !!}</a>
              </li>
            @endforeach
          </ul>
        </nav>
